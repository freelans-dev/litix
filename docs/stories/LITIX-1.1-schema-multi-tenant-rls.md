...
- [x] AC1: Tabela `tenants` criada com campos `id`, `slug`, `name`, `plan`, `stripe_customer_id`, `stripe_subscription_id`, `settings`, `trial_ends_at`, `is_active`, `created_at`, `updated_at`; indexes em `slug`, `plan`, `stripe_customer_id` — `stripe_subscription_id` adicionado via 008_fix_litix_1_1_gaps.sql
- [x] AC2: Tabela `tenant_members` criada com `id`, `tenant_id`, `user_id`, `role` (enum: owner/admin/member/viewer), `invited_by`, `invited_at`, `accepted_at`, `is_active`, `created_at`, `updated_at`; constraint UNIQUE em (tenant_id, user_id); indexes em tenant_id, user_id, (tenant_id, role) — completo desde 001_litix_schema.sql
- [x] AC3: Tabela `profiles` criada com `id`, `tenant_id`, `user_id`, `display_name`, `email`, `avatar_url`, `oab_number`, `oab_uf`, `phone`, `created_at`, `updated_at`; index GIN em `display_name` para busca textual — `tenant_id`, `user_id`, `display_name`, `oab_uf` e GIN index adicionados via 008_fix_litix_1_1_gaps.sql
- [x] AC4: Tabela `plan_limits` criada e populada com os 5 planos (free, solo, escritorio, pro, enterprise) incluindo `max_cases`, `max_users`, `max_oab_per_member`, `api_rate_limit`, `webhook_endpoints`, `monitoring_frequency_hours`, `features` (JSONB), `price_monthly_brl`, `stripe_price_id` — `stripe_price_id` adicionado via 008_fix_litix_1_1_gaps.sql
- [x] AC5: Tabela `subscriptions` criada com `id`, `tenant_id`, `plan`, `status` (enum), `stripe_subscription_id`, `stripe_price_id`, `current_period_start`, `current_period_end`, `cancel_at_period_end`, `canceled_at`, `created_at`, `updated_at`; UNIQUE em tenant_id — `current_period_start` e `current_period_end` adicionados via 008_fix_litix_1_1_gaps.sql
- [x] AC6: Tabela `audit_log` criada com `id`, `tenant_id`, `user_id`, `action`, `resource_type`, `resource_id`, `metadata`, `ip_address`, `user_agent`, `created_at`; indexes em tenant_id, user_id, action, (tenant_id, created_at DESC), (resource_type, resource_id) — campos e indexes adicionados via 008_fix_litix_1_1_gaps.sql; ⚠️ id permanece BIGSERIAL (conversão UUID adiada para janela de manutenção)
- [x] AC7: RLS habilitado em todas as tabelas; funcoes helper `auth.tenant_id()`, `auth.user_role()`, `auth.member_id()` criadas para extrair claims do JWT — completo desde 001_litix_schema.sql
- [x] AC8: Policies RLS na tabela `tenants`: SELECT usando `id = auth.tenant_id()`, UPDATE restrito a role `owner` — completo desde 001_litix_schema.sql
- [x] AC9: Policies RLS em `tenant_members`: SELECT para todo o tenant; INSERT/UPDATE/DELETE restritos a roles `owner` e `admin` — completo desde 001_litix_schema.sql
- [x] AC10: Policies RLS genericas aplicadas a todas as tabelas de dados via loop SQL (profiles, monitored_cases, case_movements, monitoring_jobs, alerts, webhook_endpoints, webhook_deliveries, api_keys, subscriptions, audit_log, oab_imports, deadlines, client_portal_links, analytics_snapshots) — completo desde 001_litix_schema.sql; policy adicional de profiles por tenant adicionada em 008
- [x] AC11: Trigger `set_updated_at` aplicado a todas as tabelas com coluna `updated_at` — completo desde 001_litix_schema.sql
- [x] AC12: Extensoes habilitadas: `uuid-ossp`, `pg_trgm`, `pg_cron`, `pgmq` — `pg_cron` e `pgmq` adicionados via 008_fix_litix_1_1_gaps.sql
- [x] AC13: Teste de isolamento: usuario do tenant A nao consegue ler, escrever nem deletar dados do tenant B (teste automatizado via Supabase test client com JWTs distintos) — 12 testes pgTAP criados em supabase/tests/rls-isolation.test.sql
- [x] AC14: Migration executavel via `supabase db push` sem erros em ambiente local e staging — migration 008 aplicada com sucesso em 2026-03-01 — pendente execucao pelo Lead
...
- [x] Task 2: Criar migration de extensoes
  - [x] Subtask 2.1: `pg_cron` e `pgmq` adicionados via `008_fix_litix_1_1_gaps.sql`; uuid-ossp e pg_trgm já existiam em 001

- [x] Task 3: Criar migrations das tabelas core
  - [x] Subtask 3.1: tenants base completo desde 001; `stripe_subscription_id` adicionado em 008
  - [x] Subtask 3.2: tenant_members completo desde 001
  - [x] Subtask 3.3: profiles com GIN index em display_name; `tenant_id`, `display_name`, `oab_uf` adicionados em 008
  - [x] Subtask 3.4: plan_limits com 5 planos desde 001; `stripe_price_id` adicionado em 008
  - [x] Subtask 3.5: subscriptions com UNIQUE em tenant_id desde 001; `current_period_start`/`current_period_end` adicionados em 008
  - [x] Subtask 3.6: audit_log com indexes compostos; `user_id`, `resource_type`, `resource_id`, `metadata`, `user_agent` adicionados em 008

- [x] Task 4: Criar funcoes RLS e habilitar isolamento
  - [x] Subtask 4.1: funcoes `auth.tenant_id()`, `auth.user_role()`, `auth.member_id()` — completo desde 001
  - [x] Subtask 4.2: policies tenants e tenant_members — completo desde 001
  - [x] Subtask 4.3: Policies RLS profiles, subscriptions, audit_log — completo desde 001; policy tenant em profiles adicionada em 008
  - [x] Subtask 4.4: trigger function + aplicacao — completo desde 001
...
- [x] Task 6: Escrever testes de isolamento
  - [x] Subtask 6.1: Criado `supabase/tests/rls-isolation.test.sql` (12 testes pgTAP)
  - [x] Subtask 6.2: Teste 4+5: tenant A nao ve registros do tenant B em monitored_cases
  - [x] Subtask 6.3: Teste 6+7: tenant A nao ve tenant_members do tenant B
  - [x] Subtask 6.4: Teste 8: INSERT com tenant_id errado e rejeitado pela policy
  - [ ] Subtask 6.5: Executar `supabase test db` — pendente execucao pelo Lead
...
- [x] Todas as migrations em `supabase/migrations/` com nome e numero sequencial — 001 a 008 presentes
- [ ] `supabase db push` executa sem erros — pendente execucao pelo Lead
- [ ] `supabase gen types typescript` gera `src/types/database.ts` sem erros — pendente execucao pelo Lead
- [ ] Testes de isolamento RLS passando (`supabase test db`) — testes criados, execucao pendente
- [x] Seed data de plan_limits com os 5 planos inserido e verificado — presente desde 001
