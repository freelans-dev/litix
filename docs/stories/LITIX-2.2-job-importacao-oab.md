...
- [x] AC1: Job `oab-import` implementado no Trigger.dev que recebe `{ tenantId, memberId, oabNumber, oabUf, importId }`
- [x] AC2: Busca executada em paralelo nos providers DataJud (busca por OAB em todos os 92 tribunais), Judit e Escavador
- [x] AC3: Resultados unificados via MergeService existente (`src/services/merge.service.ts`) com deduplicacao por CNJ
- [x] AC4: Cada processo encontrado inserido em `monitored_cases` com campos: `cnj`, `tenant_id`, `imported_by_member_id`, `tribunal`, `provider_origin`, `raw_data` (JSONB), `is_active=true`, `monitor_enabled=true`
- [x] AC5: Provider attribution preservada: `provider_origin` registra de qual provider o processo veio (ou 'merged' se de varios)
- [x] AC6: Limite do plano respeitado: se tenant ja tem `max_cases` processos, importacao para ao atingir o limite e registra aviso
- [x] AC7: Progresso atualizado em `oab_imports` a cada 10% de progresso (`imported`, `total_found`, `status`)
- [x] AC8: Supabase Realtime notifica o frontend sobre atualizacoes em `oab_imports` para progress bar em tempo real
- [x] AC9: Se job falha (timeout, erro de provider), `oab_imports.status = 'failed'` com detalhes do erro
- [x] AC10: Job idempotente: se CNJ ja existe em `monitored_cases` do tenant, atualiza dados mas nao duplica
- [x] AC11: Respeita limites de rate dos providers (circuit breaker existente)
...
- [x] Task 1: Criar migration de `monitored_cases`
  - [x] Subtask 1.1: Migration com schema completo, indexes, RLS, trigger updated_at

- [x] Task 2: Migrar providers existentes
  - [x] Subtask 2.1: Copiar e adaptar `src/providers/datajud/` para Next.js (remover Express deps)
  - [x] Subtask 2.2: Copiar e adaptar `src/providers/judit/`
  - [x] Subtask 2.3: Copiar e adaptar `src/providers/escavador/`
  - [x] Subtask 2.4: Adaptar `orchestrator.service.ts` e `merge.service.ts`

- [x] Task 3: Implementar job Trigger.dev
  - [x] Subtask 3.1: Instalar `@trigger.dev/sdk` e configurar `trigger.config.ts`
  - [x] Subtask 3.2: Implementar `src/trigger/oab-import.ts` com logica completa
  - [x] Subtask 3.3: Implementar helper `createServiceClient()` para uso nos jobs

- [x] Task 4: Configurar Supabase Realtime
  - [x] Subtask 4.1: Habilitar Realtime na tabela `oab_imports` no Supabase Dashboard
  - [x] Subtask 4.2: Hook `use-import-progress.ts` no frontend que subscreve changes

- [x] Task 5: Testes
  - [x] Subtask 5.1: Job com OAB sem processos completa com `total_found=0`
  - [x] Subtask 5.2: Deduplicacao: mesma OAB importada 2x nao duplica processos
  - [x] Subtask 5.3: Limite de plano: para ao atingir `max_cases`
  - [x] Subtask 5.4: Falha de provider primario aciona fallback

---

## Definition of Done

- [x] Tabela `monitored_cases` criada com RLS
- [x] Providers migrados e funcionando no contexto Next.js
- [x] Job `oab-import` implementado e testavel localmente
- [x] Progresso atualizado em tempo real via Supabase Realtime
- [x] Testes de integracao passando
